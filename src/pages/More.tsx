...
export const More: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col px-4 py-6 bg-zinc-955 text-left animate-in fade-in pb-20">
      ...
      <div className="glass border border-zinc-850 rounded-[28px] p-6 mb-5 flex flex-col items-center text-center">
        <img 
          src={user.avatarUrl} 
          className="h-20 w-20 rounded-full object-cover border-2 border-brand-500/40 shadow-xl mb-3.5" 
          alt="Avatar" 
        />
        <h3 className="font-extrabold text-white text-lg">{user.fullName}</h3>
        
        <div className="flex items-center gap-2 mt-2 text-[10px] text-zinc-400 font-bold">
          <button 
            onClick={handleSwitchMode}
            className="px-3 py-1 rounded-full bg-brand-500/20 text-brand-300 border border-brand-500/30 hover:bg-brand-500/30 font-extrabold uppercase tracking-wider cursor-pointer transition-all"
            title="Click to switch profiles"
          >
            Mode: {activeMode === 'seeker' ? 'Service Seeker' : 'Artisan Provider'} (Switch)
          </button>
        </div>
      </div>

      {/* Menu Options Group 1 */}
      <div className="glass border border-zinc-850 rounded-[28px] overflow-hidden mb-5">
        <div className="flex flex-col">
          <button 
            onClick={() => navigate('/settings')} 
            className="flex items-center justify-between w-full p-4 hover:bg-zinc-850/50 transition-colors text-left cursor-pointer"
          >
            <div className="flex items-center gap-3.5">
              <Setting size={18} color="currentColor" variant="Broken" className="text-brand-400" />
              <span className="text-white text-xs font-extrabold">Account Settings</span>
            </div>
          ...
          <div className="flex items-center justify-between w-full p-4 transition-colors text-left">
            <div className="flex items-center gap-3.5">
              <NotificationBing size={18} color="currentColor" variant="Broken" className="text-brand-400" />
              <span className="text-white text-xs font-extrabold">Push Notifications</span>
            </div>
            <Switch
            ...
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-3.5">
                <div className="relative flex items-center justify-center">
                  <Card size={18} color="currentColor" variant="Broken" className="text-brand-400" />
                  {hasWalletNotification && (
                    <span className="absolute -top-0.5 -right-0.5 h-2 w-2 rounded-full bg-red-500 ring-1 ring-zinc-950" />
                  )}
                </div>
                <span className="text-white text-xs font-extrabold">My Wallet Ledger</span>
              </div>
              <ArrowRight2 size={14} color="currentColor" variant="Broken" className="text-zinc-500" />
            </div>
          </button>
          ...
          >
            <div className="flex items-center gap-3.5">
              <InfoCircle size={18} color="currentColor" variant="Broken" className="text-brand-400" />
              <span className="text-white text-xs font-extrabold">Help & Support</span>
            </div>
            <ArrowRight2 size={14} color="currentColor" variant="Broken" className="text-zinc-500" />
          </button>
        </div>
      ...
      <div className="glass border border-zinc-850 rounded-[28px] overflow-hidden">
        <div className="flex flex-col">
          <button 
            ...
            disabled={checking}
            className="flex items-center justify-between w-full p-4 hover:bg-zinc-850/50 transition-colors text-left disabled:opacity-80 cursor-pointer"
          >
            <div className="flex items-center gap-3.5">
              <Refresh size={18} color="currentColor" variant="Broken" className={`text-brand-400 ${checking ? 'animate-spin' : ''}`} />
              <span className="text-white text-xs font-extrabold">Check PWA Updates</span>
            </div>
            {checking ? (
              <Spinner size="sm" color="current" className="text-brand-400 mr-1" />
            ) : (
              <span className="text-[9px] text-brand-300 bg-brand-500/10 border border-brand-500/20 px-2 py-0.5 rounded-md mr-1 font-bold">v1.3.0</span>
            )}
          </button>
          ...
          >
            <div className="flex items-center gap-3.5">
              <Logout size={18} color="currentColor" variant="Broken" className="text-red-400" />
              <span className="text-white text-xs font-extrabold">Log Out</span>
            </div>
            <ArrowRight2 size={14} color="currentColor" variant="Broken" className="text-zinc-500" />
          </button>
        </div>
